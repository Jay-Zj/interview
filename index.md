...
### 3.2.1.数组操作的常用方法：
    会改变原数组的方法：
        1.arr.pop() //pop()把Array的最后一个元素删除掉：[1,2,3,4] （此方法改变原数组）
        2.arr.push(5,6) // push()向Array的末尾添加若干元素.[1,2,3,4,5,6] （此方法改变原数组）
        3.a.unshift(-1,0) // 如果要往Array的头部添加若干元素，使用unshift()方法.[-1,0,1,2,3,4,5,6] （此方法改变原数组）
        4.arr.shift() //shift()方法则把Array的第一个元素删掉[0,1,2,3,4,5,6] （此方法改变原数组）
        5.arr.splice(index,number,item) //删除数组指定位置的几个数据，此方法会改变原数组
        arr.reverse() //会反转数组。
    不会改变原数组的方法：
        Array.isArray(arr)//判断一个变量是数组还是伪数组,是数组返回true,否则返回flase
        arr.indexOf(6) // -1 在数组中寻找是否存在传入的参数，不存在返回-1，存在返回索引值。
        arr.concat(['a','b']) //concat()方法把当前的Array和另一个Array连接起来，并返回一个新的Array：[1,2,3,4,5,'a','b'] （此方法不改变原数组，返回一个新数组）
        arr.slice(0, 3); // 从索引0开始，到索引3结束，但不包括索引3: [1, 2, 3]。 （此方法不改变原数组，返回一个新数组）
        arr.sort() //sort()可以对当前Array进行排序，它会直接修改当前Array的元素位置，直接调用时，按照默认顺序排序：
        arr.join() //将数组根据传入字符转化成字符串。
...
### 3.2.5 查找数组重复几次的计数，并用对象表示：

## 3.3 字符串
### 3.3.1 字符串操作的常用方法(必须熟记)：
    split //根据传入参数将字符串转化为数组
    charAt //根据字符下标查找字符并返回
    substr //更据传入的起始下标和结束下标切割字符串并返回
    toUpperCase //将字符转化为大写字母并返回
    toLowwerCase //将字符转化为xiao写字符并返回
    indexOf //查找字符串中指定字符，找到了返回下标，没找到返回-1
...
8.有哪几种存储数据的方式？他们之间有什么不同？ 
### 3.9.3 跨域有哪几种方式？jsonp的原理是什么：
    同源策略：
    跨域的几种方式：
    jsonp的原理：
### 3.9.4 ajax的原生写法
...
## vue基础
### 1.vue的生命周期有哪些，他们之间的作用，在有父子组件的情况下又是怎样的：
![vue生命周期](./images/life)
    在有父子组件的情况下的生命周期：
        (创建渲染过程)
...
### 2.页面第一次加载可以会触发哪些生命周期函数？
    第一次页面加载时会触发 beforeCreate, created, beforeMount, mounted 这几个钩子

### 3.computed和watch的区别，以及他们的应用场景：
...
## vue插件
### 0.vue-router的原理：
### 1.vue-router 的钩子函数有哪些？那个是可以全局使用的钩子函数？
### 1.vue-router 的hash,和history两种模式？
    
### 3.$router和$route有什么区别？
### 2.axios拦截器的使用：
### 4.axios拦截器：
### 5.axios底层是如何实现的：
### 6.vuex的原理及使用：
...
### 6.babel-tuntime和babel-polyfill的区别：

****

# 六：网络（http）
### 1.get和post的区别：
### 2.http请求头中的content-type有什么作用:
### 3.http和https之间的区别：
### 4.常见状态码：
### 5.http的三次握手和四次挥手：
### 6.跨域的几种方式，jsonp的原理：
...
# 十一.题目：
    ...
    手写代码题：
        防抖，节流，实现call,bind,call,数组去重，数组排序,深拷贝，浅拷贝
