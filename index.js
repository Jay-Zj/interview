...
//五：数组去重
//方法一：
function unquied(arr = []) {
    let resArr = [];
    arr.forEach((item => {
        if (resArr.indexOf(item) < 0) {
            resArr.push(item)
        }
    }))
    return resArr;
...
//方法二：将数组转化成为字符串
let newArr = [...new Set(a.toString().split(',').map(item => Number(item)))] //[1,2,3,4,5,6,7,8];
...
//三：防抖：函数需要被频繁触发，在一段时间内，函数只执行最后一次
